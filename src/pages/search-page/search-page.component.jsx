import React, { useContext } from 'react'
import { Link } from "react-router-dom"

import "./search-page.styles.css"
import { MainContext } from '../../context/mainContext/mainContext'
import { fetchAnything } from "../../api-fetching/api-fetching"
import TracksList from '../../reusable/tracks-list/tracks-list.component';

export default function SearchPage() {
    const {searchResult, searchValue, token, setArtistInfo, setPlaylistTracks } = useContext(MainContext);
    console.log(searchResult);

    const defaultImage = "data:image/png;base64,";


    return (
        <div className="container">
            <div className="text-center">
                <h3>Result for: <span> {searchValue} </span></h3>
            </div>
            <div className="row mt-5">
                <div className="col-md-3 search-artist-inform">
                    <div className="text-center">
                            <h3>Artists</h3>
                    </div>
                    <ul className="search-artist d-flex justify-content-between">
                    {
                        searchResult.artists ? searchResult.artists.items.map(artist => (
                            <li onClick={() => fetchAnything(token, artist.href,setArtistInfo )} key={artist.id} className="d-flex flex-column">
                                <Link to={`/artist`} >
                                    <img src={artist.images[0] ? artist.images[0].url : defaultImage} alt="artist"/>
                                    <div className="text-center">
                                        <p> {artist.name} </p>
                                    </div>
                                </Link>
                            </li>
                        )) : (
                            <p> Loading</p>
                        )
                    }
                    </ul>
                </div>
                <div className="col-md-6 px-5 search-tracks-inform">
                    <div className="text-center">
                        <h3>Top Tracks</h3>
                    </div>
                {
                    searchResult.tracks ?  <TracksList tracks={searchResult.tracks.items} albumImageUrl={defaultImage} /> : (
                        <p>Loading...</p>
                    )
                }
                </div>

                <div className="col-md-3 search-playlist-inform">
                    <div className="text-center">
                            <h3>Playlists</h3>
                    </div>
                    <ul className="search-playlist d-flex justify-content-between">
                    {
                        searchResult.playlists ? searchResult.playlists.items.map(playlist => (
                            <li onClick={() => fetchAnything(token, playlist.href,setPlaylistTracks )} key={playlist.id} className="d-flex flex-column">
                                <Link to={`/playlist/${playlist.id}`} >
                                    <img src={playlist.images[0] ? playlist.images[0].url : defaultImage} alt="artist"/>
                                    <div className="text-center">
                                        <p> {playlist.name} </p>
                                    </div>
                                </Link>
                            </li>
                        )) : (
                            <p> Loading</p>
                        )
                    }
                    </ul>
                </div>

            </div>
        </div>
    )
}
